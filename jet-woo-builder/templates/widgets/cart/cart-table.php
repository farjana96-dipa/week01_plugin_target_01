...
<form class="woocommerce-cart-form" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
	...
	<table class="shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
		<thead>
			<tr>
				...
				foreach ( $table_settings['items'] as $item ) {
					printf(
						'<th class="table-heading-%s elementor-repeater-item-%s">%s</th>',
						esc_attr( $item['cart_table_items'] ),
						esc_attr( $item['_id'] ),
				...
				<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
					...
					foreach ( $table_settings['items'] as $item ) {
						switch ( $item['cart_table_items'] ) {
							case 'remove':
								echo '<td class="product-remove elementor-repeater-item-' . esc_attr( $item['_id'] ) . '">';
							...
							case 'thumbnail':
								echo '<td class="product-thumbnail elementor-repeater-item-' . esc_attr( $item['_id'] ) . '">';
							...
							case 'name':
								echo '<td class="product-name elementor-repeater-item-' . esc_attr( $item['_id'] ) . '" data-title="' . esc_html__( $item['cart_table_heading_title'], 'jet-woo-builder' ) . '">';
							...
							case 'price':
								echo '<td class="product-price elementor-repeater-item-' . esc_attr( $item['_id'] ) . '" data-title="' . esc_html__( $item['cart_table_heading_title'], 'jet-woo-builder' ) . '">';
							...
							case 'quantity':
								echo '<td class="product-quantity elementor-repeater-item-' . esc_attr( $item['_id'] ) . '" data-title="' . esc_html__( $item['cart_table_heading_title'], 'jet-woo-builder' ) . '">';
							...
							case 'subtotal':
								echo '<td class="product-subtotal elementor-repeater-item-' . esc_attr( $item['_id'] ) . '" data-title="' . esc_html__( $item['cart_table_heading_title'], 'jet-woo-builder' ) . '">';
							...
							case 'custom_field':
								echo '<td class="product-custom-field elementor-repeater-item-' . esc_attr( $item['_id'] ) . '" data-title="' . esc_html__( $item['cart_table_heading_title'], 'jet-woo-builder' ) . '">';
								echo wp_kses_post( jet_woo_builder_template_functions()->get_cart_table_custom_field_value( $_product, $item ) );
								echo '</td>';
		...
		<tr <?php echo esc_attr( $row_styles ); ?> >
